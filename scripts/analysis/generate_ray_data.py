...
def generate_ray_data(model, output, ray_data_file, data_loc = '.', \
    ...
    trident.add_ion_list(ds, ions = ion_list)
    ...
    ad.set_field_parameter('center', gcenter*code_unit_conversion)               
    ...
    for i in range(len(1)):
        ...
        h5file = h5.File('%s/ray_%s_%i_%i.h5'%(data_loc, model, output, ray_id), 'a')
        ...
        ray = trident.make_simple_ray(ds,
                            ...
                            redshift=redshift,
                            data_filename='%s/ray_%s_%s_%i.h5'%(spectrum_directory, model, output, ray_id_list[i]))
